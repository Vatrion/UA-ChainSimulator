...
HOST = '0.0.0.0'  # Bind to specific IP for S3
PORT = 5000         # Port to listen on
SERVER_NAME = "FinalPayloadTestServer"
EXTERNAL_HOSTNAME = "well.vatrion.com" # Hostname to use in Location headers
...
PAYLOAD_CONFIG = {
    '/': {
        'technique': 'Index page for payload server',
        'default_response': {'status': HTTPStatus.OK, 'headers': {'Content-Type': 'text/html'}, 'body': b'<html><body><h1>Payload Server</h1><p>Serves the final payloads in the redirect chain.</p></body></html>'}
    },
    
    # --- Payload Points from test_config.txt ---
    '/payload1': {
    ...
    '/payload4': {
        ...
        'html_body': '<html><body>Download: <a href="/file.jsp?param=%D0%90">file.jsp</a></body></html>',
        'file_mode': True,
        'file_path': 'file.jsp',
        'default_response': {
            'headers': {'Content-Type': 'text/plain'},
    ...
    # --- Additional Payload Points ---
    '/payload5': {
        'technique': 'Base64-encoded JavaScript payload',
        'default_response': {
            'body': b'PHNjcmlwdD5hbGVydCgnZXZpbCcpPC9zY3JpcHQ+',
            'headers': {'Content-Type': 'text/plain'},
            'status': 200
        }
    },
    '/payload6': {
        'technique': 'Multiple suspicious file extensions',
        'default_response': {
            'body': b'Simulated malicious content with multiple extensions.',
            'headers': {'Content-Type': 'application/octet-stream', 'Content-Disposition': 'attachment; filename="malware.exe.php.rar"'},
            'status': 200
        }
    },
    '/malicious.jsp': {
        'technique': 'Direct access to JSP file',
        'default_response': {
            'body': b'<%@ page language="java" %><% Runtime.getRuntime().exec("calc.exe"); %>',
            'headers': {'Content-Type': 'text/plain'},
            'status': 200
        }
    },
    '/file.jsp': {
        'technique': 'JSP file referenced by payload4',
        'default_response': {
            'body': b'<%@ page language="java" %><% Runtime.getRuntime().exec("calc.exe"); %>',
            'headers': {'Content-Type': 'text/plain'},
            'status': 200
    ...
    '/%E2%80%8Bmalware.exe': {
        'technique': 'Executable with zero-width space in filename',
        'default_response': {
            'body': b'Simulated malicious EXE content with unicode filename',
            'headers': {'Content-Type': 'application/octet-stream'},
            'status': 200
        }
    },
    '/../../etc/passwd': {
        'technique': 'Path traversal payload simulation',
        'default_response': {
